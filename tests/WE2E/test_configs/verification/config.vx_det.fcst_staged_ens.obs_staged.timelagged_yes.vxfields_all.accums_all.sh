...
CYCL_HRS=( "00" )
#
# This test assumes the forecast files are staged (i.e. not generated as
# part of this experiment by running the forecast model).  Thus, turn
# off running of the forecast, post-processing, and other tasks that are
# not needed for generation of post-processed forecast files.
#
...
#
# Since the forecast files are staged and use a naming convention that
# is different from the default in the SRW App, explicitly specify the
# base staging directory of these files as well as their subdirectory
# and file templates.
#
...
#
# Assume the post-processed forecast output is in an ensemble directory
# structure (regardless of whether it is staged or will be generated by
# the current experiment).  Also, specify the number of ensemble members.
#
IS_ENS_FCST="TRUE"
NUM_ENS_MEMBERS="2"
#
